...
function renderReference(schemas, transport) {
  ...
  const lines = [
    ...
    "",
    ".Invoke failures stop dependent statements under the session's default Stop policy. For an expected failure, use .TryInvoke and branch on .Ok; inspect .Error for details. Cancellation and expired execution identity still stop execution. Do not retry a write whose execution outcome is unknown.",
    "",
